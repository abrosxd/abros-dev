...
.then(data => {
    ...
    const selectCoin = coin => {
        ...
        const selectedButton = Array.from(document.querySelectorAll('.tab-menu button')).find(button => button.textContent === coin);
    ...
    const copyAddress = () => {
        ...
        switch (currentLanguage) {
            case 'EN':
                abrosnoti.create("abros", "Address copied", `The address has been copied to the clipboard: ${addressInput.value}`, 5);
                break;
            case 'RU':
                abrosnoti.create("abros", "Адрес скопирован", `В буфер обмена был скопирован адрес: ${addressInput.value}`, 5);
                break;
            case 'PL':
                abrosnoti.create("abros", "Adres został skopiowany", `Adres został skopiowany do schowka: ${addressInput.value}`, 5);
                break;
            default:
                break;
        }
    ...
    cardHeaders.forEach(header => {
        header.addEventListener("click", function() {
            ...
            document.querySelectorAll(".aboutcard .faq-header").forEach(item => item.classList.remove("active"));
            document.querySelectorAll(".aboutcard .faq-content").forEach(item => item.classList.remove("active"));
            if (!isActive) {
    ...
    const changeabout = language => {
        ...
        document.querySelector('.tpaybutton').textContent = data.pay.button[language];
        ...
        document.querySelector('.tfacts').textContent = data.randomfacts.title[language];
        document.querySelector('.tfactsbutton').textContent = data.randomfacts.button[language];
        document.querySelector('.tsoft').textContent = data.tools.soft[language];
        document.querySelector('.tmaterials').textContent = data.tools.materials[language];
    };
