...
function openPopup(id) {
  ...
  const buttonHtml = `<a class="button" href="${project.url}" target="_blank">${buttonText}</a>`;
  const closeHtml = `<span class="button close-btn" id="close-btn" onclick="closePopup()">
  ...
  popupMenu.innerHTML = `${buttonHtml}${closeHtml}`;
...
.then(data => {
  ...
  const changefilters = language => {
    for (let key in data.filters) {
      const element = document.querySelector(`.tFilter${key}`);
      if (element) {
        element.textContent = data.filters[key][language];
      }
    }
  };
...
document.querySelector('.filter-wrap.submenu').addEventListener('click', handleFilterClick);

const handleActiveChange = () => {
  const rotation = filterSubMenu.classList.contains('active') ? 'rotate(180deg)' : 'rotate(0deg)';
  filterArrow.style.transform = rotation;
};
